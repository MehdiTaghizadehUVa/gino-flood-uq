...
apptainer exec --nv ${APPTAINER_BIND_ARGS} "${CONTAINER}" python -m neuralop.flood.cli.eval_neon_stage2   --config ${BASE}/config/coast_fgn_neon_tr450.yaml   --stage2-checkpoint ${BASE}/tr_n450/neon_stage2_best.pt   --stage1-bundle /scratch/jrj6wm/GINO_Model/model_bundles/coastal_fgn_60_calibrated_v1_20260510/coastal_fgn_bundle.json   --output-dir ${BASE}/mem_bisect/r4 --families val --m-eval 32 --k-eval 50 --seed 0   --cache-dir ${BASE}/feature_cache_tr_k50_eval --max-families 1   --impact-metrics --write-artifacts --variance-maps 1
echo "verify exit=$?"

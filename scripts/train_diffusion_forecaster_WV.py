...
def _resolve_normalizer_path(config: Any) -> Optional[Path]:
    normalizer_path = _safe_get(_safe_get(config, "data", {}), "normalizer_path", None)
...
def _prepare_datasets(
    ...
    logger,
) -> Tuple[DataLoader, DataLoader, Dict[str, Any], Path, int]:
    data_cfg = _safe_get(config, "data", {})
    seed = int(_safe_get(_safe_get(config, "distributed", {}), "seed", 123))
    static_text_files = list(_safe_get(data_cfg, "static_text_files", ["M40_CS.txt", "M40_CU.txt", "M40_FA.txt"]))

    full_dataset = FloodDatasetHDF(
        data_root=str(_safe_get(data_cfg, "root", "")),
        n_history=int(_safe_get(data_cfg, "n_history", 3)),
        train_txt=str(_safe_get(data_cfg, "train_txt", "train.txt")),
        write_train_txt=bool(_safe_get(data_cfg, "write_train_txt", False)),
        static_text_files=static_text_files,
        hdf_suffix=".hdf",
        noise_type=str(_safe_get(data_cfg, "noise_type", "none")),
        noise_std=_safe_get(data_cfg, "noise_std", None),
        skip_before_timestep=int(_safe_get(data_cfg, "skip_before_timestep", 0)),
        ar_rollout_steps=max(1, int(_safe_get(_safe_get(config, "opt", {}), "ar_rollout_steps", 1))),
        target_variables=target_variables,
    )

    n_samples_max = _safe_get(data_cfg, "n_samples_max", None)
    ...
    if normalizer_path is not None and normalizer_path.exists():
        ...
        normalizers = fit_normalizers_streaming(
            train_raw,
            chunk_size=int(_safe_get(data_cfg, "normalizer_chunk_size", 10000)),
            expect_target=True,
        )
    ...
    query_res = list(_safe_get(data_cfg, "query_res", [48, 48]))
    ...
    batch_size = int(_safe_get(data_cfg, "batch_size", 8))
    num_workers = int(_safe_get(data_cfg, "num_workers", 0))
    pin_memory = bool(_safe_get(data_cfg, "pin_memory", True))
    persistent_workers = bool(_safe_get(data_cfg, "persistent_workers", False)) and num_workers > 0
    prefetch_factor = _safe_get(data_cfg, "prefetch_factor", 2)
    ...
    return train_loader, test_loader, normalizers, normalizer_path, n_static
...
def _configure_denoiser(
    ...
    n_history = int(_safe_get(_safe_get(config, "data", {}), "n_history", 3))

    base_channels = n_static + n_history * 1 + n_history * n_target_channels
    extra = 0
    if bool(_safe_get(cond_cfg, "add_noisy_target", True)):
        extra += n_target_channels
    if bool(_safe_get(cond_cfg, "add_time_features", True)):
        t_type = str(_safe_get(cond_cfg, "time_feature_type", "sincos")).lower()
        extra += 2 if t_type == "sincos" else 1

    total_in_channels = base_channels + extra

    setattr(config, "arch", "gino")
    gino = _safe_get(config, "gino", {})
    setattr(gino, "data_channels", int(total_in_channels))
    setattr(gino, "out_channels", int(n_target_channels))
    setattr(gino, "output_distribution", "deterministic")
    setattr(gino, "use_fgn_noise", False)
    if str(_safe_get(gino, "fno_norm", "")).lower() == "ada_in":
        setattr(gino, "fno_norm", "instance_norm")
    return base_channels, total_in_channels


def _init_scheduler(config: Any, optimizer: torch.optim.Optimizer):
    opt = _safe_get(config, "opt", {})
    name = str(_safe_get(opt, "scheduler", "")).lower()
    if name == "reducelronplateau":
        return torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode=str(_safe_get(opt, "scheduler_mode", "min")),
            patience=int(_safe_get(opt, "scheduler_patience", 5)),
            threshold=float(_safe_get(opt, "scheduler_threshold", 1e-4)),
            threshold_mode=str(_safe_get(opt, "scheduler_threshold_mode", "rel")),
            cooldown=int(_safe_get(opt, "scheduler_cooldown", 0)),
            min_lr=float(_safe_get(opt, "scheduler_min_lr", 0.0)),
        )
    if name == "cosineannealinglr":
        return torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=int(_safe_get(opt, "scheduler_T_max", 200)),
            eta_min=float(_safe_get(opt, "scheduler_eta_min", 0.0)),
        )
    if name == "steplr":
        return torch.optim.lr_scheduler.StepLR(
            optimizer,
            step_size=int(_safe_get(opt, "step_size", 50)),
            gamma=float(_safe_get(opt, "gamma", 0.5)),
        )
    return None
...
def _build_wandb_names(config: Any, seed: int) -> Tuple[str, str]:
    ...
    gp_type = str(_safe_get(gp_cfg, "type", "rff_rbf"))
    ls = float(_safe_get(gp_cfg, "length_scale", 0.05))
    n_steps = int(_safe_get(sampler_cfg, "num_steps", 40))
...
def _maybe_init_wandb(config: Any, seed: int, logger) -> Optional[Any]:
    wb_cfg = _safe_get(config, "wandb", {})
    if not bool(_safe_get(wb_cfg, "log", False)):
        return None
    ...
    run = wandb.init(
        project=str(_safe_get(wb_cfg, "project", "Flood_GINO_NoPhysics")),
        entity=_safe_get(wb_cfg, "entity", None),
        group=str(_safe_get(wb_cfg, "group", group_default) or group_default),
        name=str(_safe_get(wb_cfg, "name", name_default) or name_default),
...
def main() -> int:
    ...
    log_file = _safe_get(config, "log_file", "train_diffusion.log")
    ...
    train_loader, test_loader, normalizers, normalizer_path, n_static = _prepare_datasets(
        config,
        target_variables=target_variables,
        logger=logger,
    )
    ...
    _, total_in_channels = _configure_denoiser(config, n_static=n_static, n_target_channels=1)
    gino_cfg = copy.deepcopy(_to_builtin(_safe_get(config, "gino", {})))

    model = get_model(config).to(device)
    logger.info("Initialized denoiser (GINO) with in_channels=%d", total_in_channels)

    diff_cfg = _safe_get(config, "diffusion", {})
    gp_cfg = _safe_get(diff_cfg, "gp", {})
    cond_cfg_raw = _safe_get(diff_cfg, "conditioning", {})
    cond_cfg = ConditioningConfig(
        add_noisy_target=bool(_safe_get(cond_cfg_raw, "add_noisy_target", True)),
        add_time_features=bool(_safe_get(cond_cfg_raw, "add_time_features", True)),
        time_feature_type=str(_safe_get(cond_cfg_raw, "time_feature_type", "sincos")),
    )
    gp_sampler = PointRFFGaussianProcessSampler(
        dim=2,
        gp_type=str(_safe_get(gp_cfg, "type", "rff_rbf")),
        sigma=float(_safe_get(gp_cfg, "sigma", 1.0)),
        length_scale=float(_safe_get(gp_cfg, "length_scale", 0.05)),
        rff_features=int(_safe_get(gp_cfg, "rff_features", 256)),
        seed=seed,
    ).to(device)

    sched_cfg = _safe_get(diff_cfg, "schedule", {})
    sampler_cfg = _safe_get(diff_cfg, "sampler", {})
    forecaster = ConditionalDDOForecaster(
        denoiser=model,
        gp_sampler=gp_sampler,
        parameterization=str(_safe_get(diff_cfg, "parameterization", "epsilon")),
        timestep_sampler=str(_safe_get(diff_cfg, "timestep_sampler", "low_discrepancy")),
        lmbd0=float(_safe_get(sched_cfg, "lmbd0", 10.0)),
        lmbd1=float(_safe_get(sched_cfg, "lmbd1", -10.0)),
        weight_method=_safe_get(sched_cfg, "weight_method", "shifted_sigmoid_2"),
        conditioning=cond_cfg,
        sampler_method=str(_safe_get(sampler_cfg, "method", "denoise")),
        sampler_num_steps=int(_safe_get(sampler_cfg, "num_steps", 40)),
        sampler_s_min=float(_safe_get(sampler_cfg, "s_min", 1e-4)),
        sampler_return_mean_last=bool(_safe_get(sampler_cfg, "return_mean_last", True)),
    ).to(device)

    opt_cfg = _safe_get(config, "opt", {})
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=float(_safe_get(opt_cfg, "learning_rate", 1e-4)),
        weight_decay=float(_safe_get(opt_cfg, "weight_decay", 1e-4)),
    )
    scheduler = _init_scheduler(config, optimizer)

    run = _maybe_init_wandb(config, seed=seed, logger=logger)

    ckpt_dir = Path(str(_safe_get(_safe_get(config, "checkpoint", {}), "save_dir", "./checkpoints_WV_depth_only_diffusion")))
    if not ckpt_dir.is_absolute():
        ckpt_dir = (_SCRIPT_DIR / ckpt_dir).resolve()
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    n_epochs = int(_safe_get(opt_cfg, "n_epochs", 1))
    print_every = int(_safe_get(config, "print_every", DEFAULT_PRINT_EVERY))
    max_val_batches = int(_safe_get(config, "max_val_batches", DEFAULT_MAX_VAL_BATCHES))
    grad_clip = _safe_get(opt_cfg, "clip_grad_norm", None)

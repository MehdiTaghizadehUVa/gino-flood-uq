...
apptainer exec --bind /scratch,/home "${CONTAINER}" \
  ...
  --cache-dir "${CACHE_DIR}" --expected-head "${HEAD}" >/dev/null
test -s "${PLAN}"
...
EXPORTS="ALL,NEON_REPO=${REPO},NEON_CONTAINER=${CONTAINER},NEON_EXPECTED_HEAD=${HEAD},NEON_SCALEOUT_PLAN=${PLAN},NEON_SCALEOUT_ROOT=${RUN_ROOT},NEON_CACHE_DIR=${CACHE_DIR}"

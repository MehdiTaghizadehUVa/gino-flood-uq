...
#SBATCH -J wv_mcd_tr
#SBATCH --array=0-0
#SBATCH -o /scratch/jrj6wm/GINO_Model/neuraloperator_runs/wv_mcdropout/slurm/wv_mcd_tr-%A_%a.out
#SBATCH -e /scratch/jrj6wm/GINO_Model/neuraloperator_runs/wv_mcdropout/slurm/wv_mcd_tr-%A_%a.err
...
RUN_ROOT="${RUN_ROOT:-/scratch/$USER/GINO_Model/neuraloperator_runs/wv_mcdropout}"
WV_MCD_SMOKE="${WV_MCD_SMOKE:-0}"
ARTIFACT_ROOT="${ARTIFACT_ROOT:-${RUN_ROOT}/artifacts}"
if [[ "${WV_MCD_SMOKE}" == "1" ]]; then
  ARTIFACT_ROOT="${SMOKE_ARTIFACT_ROOT:-${RUN_ROOT}/smoke/artifacts}"
fi
...
if [[ "${WV_MCD_SMOKE}" == "1" ]]; then
  ...
  if [[ ! -f "${SMOKE_TRAIN_TXT}" ]]; then
    head -n "${SMOKE_RUN_IDS:-8}" "${DATA_ROOT}/train.txt" > "${SMOKE_TRAIN_TXT}"
  fi
...
if [[ "${STRUCTURAL_DRY_POLICY}" == "masked_primary" ]]; then
  CLI_ARGS+=(
    ...
    --structural_dry.canonical_data_root "${DATA_ROOT}"

...
def main():
    ...
    for flag in ["--out_dir", "--sample_index", "--target_timestep", "--seed"]:
        if flag in sys.argv:
    ...
    from train_gino_flood_train_rollout_animation_WV import (
    ...
    save_dir = args.save_dir or getattr(config.checkpoint, "save_dir", ".")
    ...
    with torch.no_grad():
        if use_fgn and fgn_noise_dim is not None:
            ...
            z = torch.randn(fgn_noise_dim, device=device, dtype=sample["x"].dtype, generator=g)

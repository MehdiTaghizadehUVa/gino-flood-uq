...
def _fit_one_bin(
    ...
    *,
    ...
    def objective(theta: Sequence[float]) -> float:
        ...
        return empirical_crps_mean(cal, ref)

    x0 = np.asarray([0.0, 1.0, 1.0], dtype=np.float64)
    best_x = x0.copy()
    best_val = objective(best_x)
    if minimize is not None:
        res = minimize(objective, x0=x0, method="Nelder-Mead", options={"maxiter": 180, "xatol": 1e-4, "fatol": 1e-5})
        candidate = np.asarray(res.x, dtype=np.float64)
        candidate = np.asarray([np.clip(candidate[i], bnds[i][0], bnds[i][1]) for i in range(3)])
        val = objective(candidate)
        if np.isfinite(val) and val <= best_val:
            best_x = candidate
            best_val = float(val)
        res2 = minimize(objective, x0=best_x, method="L-BFGS-B", bounds=bnds, options={"maxiter": 120})
        if getattr(res2, "success", False) or np.isfinite(getattr(res2, "fun", np.nan)):
            candidate = np.asarray(res2.x, dtype=np.float64)
            val = objective(candidate)
            if np.isfinite(val) and val <= best_val:
                best_x = candidate
                best_val = float(val)
    else:  # Conservative fallback: coarse coordinate candidates around identity.
        for a in np.linspace(bnds[0][0], bnds[0][1], 9):
            for beta in np.linspace(max(0.5, bnds[1][0]), min(1.5, bnds[1][1]), 9):
                for gamma in np.linspace(max(0.5, bnds[2][0]), min(1.8, bnds[2][1]), 9):
                    val = objective((a, beta, gamma))
                    if val < best_val:
                        best_val = val
                        best_x = np.asarray([a, beta, gamma], dtype=np.float64)
    return {
        "a_m": float(best_x[0]),
        "beta": float(best_x[1]),
    ...
    *,
    ...
    for lead_idx in range(n_l):
        ...
        if pred_l.shape[1] >= int(min_fit_points_per_bin):
            lead_fits.append(_fit_one_bin(pred_l, ref_l, bounds=bounds))
        else:
            lead_fits.append({**global_fit, "fallback": "global"})

    coeff = np.zeros((n_l, n_w, 3), dtype=np.float64)
    objective = np.full((n_l, n_w), np.nan, dtype=np.float64)
    ...
    for lead_idx in range(n_l):
        for wet_idx in range(n_w):
            ...
            if pred_b.shape[1] >= int(min_fit_points_per_bin):
                fit = _fit_one_bin(pred_b, ref_b, bounds=bounds)
            ...
            objective[lead_idx, wet_idx] = float(fit.get("objective", np.nan))

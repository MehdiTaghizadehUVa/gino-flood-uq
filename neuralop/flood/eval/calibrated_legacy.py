...
from neuralop.flood.cli.train_operator import (  # noqa: E402
...
from neuralop.training.leadtime_affine_calibration import (  # noqa: E402
...
def _resolve_split_guard_root(config: Any, section: str) -> Path:
...
def _rollout_prediction_per_hydrograph(
    ...
    calibration_coeffs_wd: Optional[Dict[str, Any]] = None,
) -> None:
    """
    ...
    if calibration_coeffs_wd is not None:
        if "wd" not in target_variables:
            raise ValueError(
                "WD calibration coefficients were provided but target_variables does not contain 'wd'."
            )
        for key in ("a", "b", "c"):
            if key not in calibration_coeffs_wd:
                raise KeyError(f"Missing calibration coefficient key: {key}")
        n_steps_coeff = int(len(np.asarray(calibration_coeffs_wd["a"]).reshape(-1)))
        if n_steps_coeff != int(rollout_length):
            raise ValueError(
                "Calibration coefficient length mismatch: "
                f"len(a)={n_steps_coeff}, rollout_length={rollout_length}."
            )
    logger.info(
    ...
    if calibration_coeffs_wd is not None:
        logger.info("Applying lead-time WD affine calibration during metric computation.")
    if gaussian_mode:
        logger.info(
    ...
    for sample in tqdm(hydrograph_samples, desc="Hydrograph rollout evaluation"):
        ...
        for t in range(rollout_length):
            ...
            if calibration_coeffs_wd is not None:
                inv_pred_ens = _apply_wd_calibration_to_prediction_ensemble(
                    inv_pred_ens,
                    target_variables,
                    a_t=float(np.asarray(calibration_coeffs_wd["a"])[t]),
                    b_t=float(np.asarray(calibration_coeffs_wd["b"])[t]),
                    c_t=float(np.asarray(calibration_coeffs_wd["c"])[t]),
                )
            ...
            for ch_idx, ch_name in enumerate(target_variables):
                ...
                if gaussian_mode and mu_phys_stack is not None and logvar_phys_stack is not None:
                    if calibration_coeffs_wd is not None and ch_name == "wd":
...
def _build_rollout_normalized_dataset(
    ...
    rollout_static = _opt(
        config, config_section, "static_text_files", DEFAULT_STATIC_FILES
    )
    if not isinstance(rollout_static, list):
        rollout_static = list(rollout_static)
    section_cfg = getattr(config, config_section)
    rollout_root = str(section_cfg.root)
    if config_section == "rollout_calibration":
        rollout_root_path = Path(rollout_root)
        if (not rollout_root.strip()) or (not rollout_root_path.exists()):
            fallback_root = str(_opt(config, "rollout_data", "root", "")).strip()
            if not fallback_root:
                raise FileNotFoundError(
                    "rollout_calibration.root is unset and rollout_data.root is unavailable."
            ...
            logger.warning(
                "rollout_calibration.root=%s is unavailable; falling back to rollout_data.root=%s",
                rollout_root,
                fallback_root,
            )
            rollout_root = fallback_root

    with _PhaseTimer(logger, "Building rollout test dataset"):
        split_txt_effective = (
            str(split_txt).strip()
            if split_txt is not None
            else str(_opt(config, config_section, "test_txt", "test.txt")).strip()
        )
        if not split_txt_effective:
            raise ValueError(
        ...
        rds = FloodRolloutTestDatasetHDF(
            rollout_data_root=rollout_root,
            n_history=history_steps,
            rollout_length=rollout_length,
            run_ids=None,
            test_txt=split_txt_effective,
    ...
    logger.info(
        "Loaded rollout split '%s' from %s/%s (%d runs).",
        split_name,
        rollout_root,
        split_txt_effective,
        len(rds),

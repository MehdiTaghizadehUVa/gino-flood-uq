  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-labelledby="calibration-title calibration-desc">
        <title id="calibration-title">Calibration curve for water depth above 0.30 meters</title>
        <desc id="calibration-desc">The raw model probability is on the horizontal axis and the calibrated probability is on the vertical axis.</desc>
        {ticks.map((tick) => (
          <g key={tick}>
        ...
        <text className="chart-axis-label" x={WIDTH / 2} y={HEIGHT - 1} textAnchor="middle">Raw model probability</text>
        <text className="chart-axis-label" transform={`translate(14 ${HEIGHT / 2}) rotate(-90)`} textAnchor="middle">Calibrated probability</text>
      </svg>
      <figcaption>
        The dots summarize separate reference evidence. The curve maps the raw ensemble probability to the calibrated probability.
      </figcaption>
    </figure>
  );
}

...
const DESCRIPTOR_LABELS: Record<string, string> = {
  ...
  peak_area_weighted_between_checkpoint_spread_wd_m: "Peak epistemic uncertainty",
  ...
  peak_between_checkpoint_disagreement_lead_hours: "Lead time to peak epistemic uncertainty",
...
const CANDIDATE_REASON_LABELS: Record<string, string> = {
  ...
  high_checkpoint_disagreement_share: "Epistemic uncertainty dominates ensemble spread",
  broad_checkpoint_disagreement_footprint: "Epistemic uncertainty covers a broad area",
...
function formatMonitoringFlag(flag: MonitoringFlag): string {
  ...
  switch (flag.code) {
    ...
    return "Epistemic uncertainty explains an unusually large share of ensemble spread.";
    ...
    .replace(/peak_area_weighted_between_checkpoint_spread_wd_m/g, "peak epistemic uncertainty")
...
export default function RunDetails() {
  ...
  const uncertaintyTiles = [
    ...
    },
    {
      ...
      caption: "Variance across the three checkpoint means, representing uncertainty in learned model structure and parameters.",
      insight: "High values mean the independently trained checkpoints produce different central responses.",
    },
    {
      id: "within_var_map.png",
      title: "Aleatoric uncertainty",
      caption: "Expected variance among latent members within each checkpoint, representing retained event variability.",
      insight: "High values mean latent members from the same checkpoint spread out even when checkpoint means are similar.",
    },
  ].filter((tile) => artifacts.some((artifact) => artifact.artifact_id === tile.id));
    ...
    <div className="shell run-console">
      ...
      {tab === "uncertainty" && (
        <UncertaintyTab>
          <section className="panel" aria-label="Uncertainty headline">
            ...
            {spreadSummary && (
              <div className="share-strip" aria-label="Epistemic versus aleatoric uncertainty share">
                ...
                <div className="share-legend">
                  <span><i className="dot dot-between" /> epistemic {(spreadSummary.between_share * 100).toFixed(0)}%</span>
                  <span><i className="dot dot-within" /> aleatoric {(spreadSummary.within_share * 100).toFixed(0)}%</span>
                </div>
              </div>
            )}
          </section>
      ...
      {summary && (
        <section className="cards">
          ...
          <article className="card">
            <span className="card-label">Epistemic uncertainty</span>
            <span className="card-value">
            ...
            <span className="card-note">
              {summary.peak_area_weighted_between_checkpoint_spread_wd_m == null
                ...
                : `${formatNumber(summary.peak_area_weighted_between_checkpoint_spread_wd_m, 3)} m epistemic spread`}
            </span>
          </article>
        </section>
      )}
